...
class MetalGraph(GraphRunner):
  def __init__(self, jit_cache: List[ExecItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]):
    ...
    icb_descriptor = msg_ios2(b"MTLIndirectCommandBufferDescriptor", "new", restype=objc_instance)
    msg_ios2(icb_descriptor, "setCommandTypes:", MTLIndirectCommandType.MTLIndirectCommandTypeConcurrentDispatch)
    msg_ios2(icb_descriptor, "setInheritBuffers:", False)
    msg_ios2(icb_descriptor, "setInheritPipelineState:", False)
    msg_ios2(icb_descriptor, "setMaxKernelBufferBindCount:", 31)

    self.icb = msg_ios2(self.device.device, "newIndirectCommandBufferWithDescriptor:maxCommandCount:options:",
    ...
    for j,ji in enumerate(self.jit_cache):
      ...
      icb_command = msg_ios2(self.icb, "indirectComputeCommandAtIndex:", j, restype=objc_instance)
      all_pipelines.append(prg.clprg.pipeline_state)
      msg_ios2(icb_command, "setComputePipelineState:", prg.clprg.pipeline_state)
      for i,b in enumerate(ji.bufs):
        if b is not None and b not in input_rawbuffers:
          msg_ios2(icb_command, "setKernelBuffer:offset:atIndex:", b._buf.buf, b._buf.offset, i)

      global_size, local_size = prg.p.launch_dims(var_vals)
      msg_ios2(icb_command, "concurrentDispatchThreadgroups:threadsPerThreadgroup:", tuple(global_size), tuple(local_size))
      msg_ios2(icb_command, "setBarrier")
  ...
  def __call__(self, input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int], wait=False) -> Optional[float]:
    ...
    for j, global_dims, local_dims in self.updated_launch_dims(var_vals):
      ...
      computeCommand = msg_ios2(self.icb, "indirectComputeCommandAtIndex:", j,restype=objc_id)
      msg_ios2(computeCommand, "concurrentDispatchThreadgroups:threadsPerThreadgroup:",
    ...
    msg_ios2(encoder, "useResources:count:usage:", all_resources, len(all_resources),
      MTLResourceUsage.MTLResourceUsageRead | MTLResourceUsage.MTLResourceUsageWrite)

    msg_ios2(encoder, "executeCommandsInBuffer:withRange:", self.icb, self.range)
